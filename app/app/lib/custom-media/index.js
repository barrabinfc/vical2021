const _memoizedCustomMedia = {};

function customMedia(string, map) {
  return customMedia.replace(
    string,
    customMedia.parse(string, _memoizedCustomMedia)
  );
}

customMedia.parse = function(string, map) {
  map = map || {};
  var RE_CM = /@custom-media\s+--([\w-]+)\s+\((.*?)\)\s*;/gm;
  var m;
  while ((m = RE_CM.exec(string))) {
    map[m[1]] = m[2];
  }
  return map;
};
