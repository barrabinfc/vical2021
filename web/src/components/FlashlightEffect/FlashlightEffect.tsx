...
export default function Flashlight() {
  ...
  const onMouseMove = (evt) => {
    ...
    const elementsBounds = refElements.current
      .map((el) => {
      ...
      .filter(
        (elBound) => elBound.right >= 0 && elBound.left <= windowW && elBound.bottom >= 0 && elBound.top <= windowH
      );
    ...
    elementsBounds.forEach((elBound, i) => {
      ...
      const [shadowOffsetX, shadowOffsetY] = [
        distanceInterp(-Math.cos(angle) * distance),
        distanceInterp(Math.sin(angle) * distance)
      ];

      const theDomRef = refElements.current[i];
      theDomRef.style.setProperty(
        'box-shadow',
        `${shadowOffsetX * 0.05}px ${shadowOffsetY * 0.05}px ${
          penumbraSpread * 0.02
        }px hsl(var(--surface-shadow) / calc(var(--shadow-strength))), ` +
          `${shadowOffsetX * 0.12}px ${shadowOffsetY * 0.12}px ${
            penumbraSpread * 0.05
          }px hsl(var(--surface-shadow) / calc(var(--shadow-strength))), ` +
          `${shadowOffsetX * 0.22}px ${shadowOffsetY * 0.22}px ${
            penumbraSpread * 0.1
          }px hsl(var(--surface-shadow) / calc(var(--shadow-strength) + 0.02)), ` +
          `${shadowOffsetX * 0.55}px ${shadowOffsetY * 0.55}px ${
            penumbraSpread * 0.2
          }px hsl(var(--surface-shadow) / calc(var(--shadow-strength) + 0.02)), ` +
          `${shadowOffsetX * 0.81}px ${shadowOffsetY * 0.81}px ${
            penumbraSpread * 0.33
          }px hsl(var(--surface-shadow) / calc(var(--shadow-strength) + 0.03)), ` +
